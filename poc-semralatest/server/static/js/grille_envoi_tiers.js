// Grille Envoi Tiers - Tableau éditable avec fonctionnalités Excel
...
(function() {
    ...
    if (!currentPath.includes('grille_envoi_tiers')) {
        console.log('[grille_envoi_tiers.js] Page non concernée, skip');
    ...
    async function loadTableData() {
        try {
            const response = await fetch('/api/grille_envoi_tiers/data');
    ...
    async function saveChanges() {
        try {
            const response = await fetch('/api/grille_envoi_tiers/save', {
                method: 'POST',
    ...
    function exportToJSON() {
        ...
        link.href = url;
        link.download = `grille_envoi_tiers_${new Date().toISOString().split('T')[0]}.json`;
